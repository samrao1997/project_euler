...
def simulation(num_games, num_turns, turns_to_burn):
    ...
    for sim in range(num_games):
        ...
        decks = {"CC": list(CC_cards), "CH": list(CH_cards)}
        ...
        for turn in range(num_turns + turns_to_burn):
            if DEBUG:
                print(
                    "[Sim %d][Turn %d] Currently on: %s (Square %d)"
                    % (sim, turn, current_square, s_to_i(current_square))
                )
            ...
            while continueRolling:
                ...
                if DEBUG:
                    print(
                        "[Sim %d][Turn %d] Rolled [%d, %d] => %d (Doubles? %r)"
                        % (sim, turn, d1, d2, d1 + d2, continueRolling)
                    )
                ...
                if continueRolling:
                    ...
                    if numDoubles == 3:
                        if DEBUG:
                            print(
                                "[Sim %d][Turn %d] Rolled three doubles. Going to JAIL."
                                % (sim, turn)
                            )
                        current_square = "JAIL"
                        continue
                ...
                if current_square == "G2J":
                    ...
                    if isinstance(card, str):
                        ...
                        card
                    ):  # Only one obvious way to do everything, duh: http://bugs.python.org/issue10518
