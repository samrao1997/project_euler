...
def largest_vert(grid):
    """
    Given a grid (in 2D array form) returns the largest productof four numbers either up or down
    """
    largest_prod = 0
...
def largest_hor(grid):
    """
    Given a grid (in 2D array form) returns the largest productof four numbers either left or right
    """
    largest_prod = 0
...
## Some preprocessing to make grid of string into 2D array of ints
# print(grid)
for i in range(len(grid)):
    grid[i] = grid[i].split(" ")
    for j in range(len(grid[i])):
